...
#define IPC_MAGIC "HYDRA-IPC"
// clang-format off
#define IPC_MAGIC_ARR { 'H', 'Y', 'D', 'R', 'A', '-', 'I', 'P', 'C' }
// clang-format on
#define IPC_MAGIC_LEN 9  // Not including null char
...
const char *DEFAULT_SOCKET_PATH = "/tmp/hydra.sock";
...
typedef enum IPCMessageType {
  ...
  IPC_TYPE_GET_HYDRA_CLIENT = 4,
  IPC_TYPE_SUBSCRIBE = 5,
  IPC_TYPE_EVENT = 6
} IPCMessageType;

// Every IPC message must begin with this
typedef struct hydra_ipc_header {
  uint8_t magic[IPC_MAGIC_LEN];
  uint32_t size;
  uint8_t type;
} __attribute((packed)) hydra_ipc_header_t;
...
send_message(IPCMessageType msg_type, uint32_t msg_size, uint8_t *msg)
{
  ...
  size_t header_size = sizeof(hydra_ipc_header_t);
...
get_hydra_client(Window win)
{
  ...
  send_message(IPC_TYPE_GET_HYDRA_CLIENT, msg_size, (uint8_t *)msg);
...
print_usage(const char *name)
{
  ...
  puts("");
  puts("  get_hydra_client <window_id>      Get hydra client proprties");
  puts("");
...
main(int argc, char *argv[])
{
  ...
  if (strcmp(argv[i], "help") == 0)
    ...
    get_layouts();
  } else if (strcmp(argv[i], "get_hydra_client") == 0) {
    if (++i < argc) {
      if (is_unsigned_int(argv[i])) {
        Window win = atol(argv[i]);
        get_hydra_client(win);
      } else
