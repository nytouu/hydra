 ...
 *
 * The event handlers of hydra are organized in an array which is accessed
...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayVisual,
	   ...
	   NetSystemTrayOrientationHorz, NetWMWindowTypeDialog, NetClientList, NetWMCheck, NetClientListStacking,
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
        ...
        if (ev->x < TEXTW(buttonbar))
			...
			x = selmon->ww - statusw - sp * 2 - getsystraywidth();
			click = ClkStatusText;
	...
	if (showsystray && cme->window == systray->win && cme->message_type == netatom[NetSystemTrayOP]) {
		...
		if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
			...
			XSelectInput(dpy, c->win, StructureNotifyMask | PropertyChangeMask | ResizeRedirectMask);
				XClassHint ch = {"hydrasystray", "hydrasystray"};
...
runautostart(void)
{
	...
	/* if $XDG_DATA_HOME is set and not empty, use $XDG_DATA_HOME/hydra,
	 * otherwise use ~/.local/share/hydra as autostart script directory
	 */
	xdgdatahome = getenv("XDG_DATA_HOME");
	if (xdgdatahome != NULL && *xdgdatahome != '\0') {
		/* space for path segments, separators and nul */
		pathpfx = ecalloc(1, strlen(xdgdatahome) + strlen(hydradir) + 2);

		if (sprintf(pathpfx, "%s/%s", xdgdatahome, hydradir) <= 0) {
			free(pathpfx);
			return;
		}
	} else {
		...
		if (sprintf(pathpfx, "%s/%s/%s", home, localshare, hydradir) < 0) {
			free(pathpfx);
			return;
		}
	}
	...
	if (! (stat(pathpfx, &sb) == 0 && S_ISDIR(sb.st_mode))) {
		 ...
		 * so we try ~/.hydra instead
		 */
		char *pathpfx_new = realloc(pathpfx, strlen(home) + strlen(hydradir) + 3);
		if(pathpfx_new == NULL) {
			free(pathpfx);
			return;
		}
   pathpfx = pathpfx_new;
		...
		if (sprintf(pathpfx, "%s/.%s", home, hydradir) <= 0) {
			free(pathpfx);
			return;
		}
	}
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "hydra", 5);
...
spawncmd(const Arg *arg)
{
	...
	if ((pid = fork()) == 0) {
		...
		fprintf(stderr, "hydra: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
	return pid;
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		/* m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by, w, bh, 0, depth, */
		                          InputOutput, visual,
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "hydra: fatal error: request code=%d, error code=%d\n",
...
{
	die("hydra: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("hydra: cannot open display");
	if (!(xcon = XGetXCBConnection(dpy)))
		die("hydra: cannot get xcb connection\n");
	checkotherwm();
	setup();
#ifdef __OpenBSD__
