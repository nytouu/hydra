                 ...
                 uint8_t **reply)
{
  ...
  const int32_t to_read = sizeof(hydra_ipc_header_t);
  char header[to_read];
  char *walk = header;
...
ipc_parse_run_command(char *msg, IPCParsedCommand *parsed_command)
{
  ...
  *argc = args_val->u.array.len;
  ...
  // function. This is just the way hydra's void(Arg*) functions are setup.
  if (*argc == 0) {
...
/**
 * Parse an IPC_TYPE_GET_HYDRA_CLIENT message from a client. This function
 ...
 */
static int
ipc_parse_get_hydra_client(const char *msg, Window *win)
{
  char error_buffer[100];
 ...
 * function succeeded based on hydra's void(const Arg*) function types. Pointer
...
/**
 * Called when an IPC_TYPE_GET_HYDRA_CLIENT message is received from a client. It
 ...
 */
static int
ipc_get_hydra_client(IPCClient *ipc_client, const char *msg, const Monitor *mons)
{
  Window win;

  if (ipc_parse_get_hydra_client(msg, &win) < 0) return -1;
    ...
    for (Client *c = m->clients; c; c = c->next)
      if (c->win == win) {
        ...
        ipc_reply_prepare_send_message(gen, ipc_client,
                                       IPC_TYPE_GET_HYDRA_CLIENT);

        return 0;
      }

  ipc_prepare_reply_failure(ipc_client, IPC_TYPE_GET_HYDRA_CLIENT,
...
{
  hydra_ipc_header_t header = {
      .magic = IPC_MAGIC_ARR, .type = msg_type, .size = msg_size};

  uint32_t header_size = sizeof(hydra_ipc_header_t);
                              ...
                              const Layout *layouts, const int layouts_len)
{
  ...
  if (ev->events & EPOLLHUP) {
    ...
    } else if (msg_type == IPC_TYPE_GET_HYDRA_CLIENT) {
      if (ipc_get_hydra_client(c, msg, mons) < 0) return -1;
